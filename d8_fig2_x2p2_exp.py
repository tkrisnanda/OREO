...
plt.xticks([-1, 0, 1])  
plt.yticks([-1, 0, 1])  
plt.clim(1.0043145750507623, 20.32106781186547)
plt.clim(0, 20)
...
data_BS_all = (1-scale_q(data_BS_all))*f#the minus sign is because our displacement is reversed
data_BS_all_new = (1-scale_q(data_BS_all_new))*f#the minus sign is because our displacement is reversed
...
data_id = np.load(r'C:\Users\tanju\Dropbox\PY\OREO\x2p2_ideal.npz')['obs']
...
plt.plot(-AL,np.fliplr(data_id).diagonal(),'k-', linewidth=1, alpha=1)
plt.tick_params(axis='both', which='major', labelsize=6)
plt.xlim([-1.05,1.05])
plt.yticks([0, 10, 20], ['1','1','1'])
# plt.savefig('fig2d_bot.pdf')
plt.show()
