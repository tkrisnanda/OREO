...
rho_cav = np.load(r"C:\Users\tanju\Dropbox\PY\OREO\rho_f0p3_weak_dephasing.npz")['rho']
...
# Model function
def cos_model(theta, A, C):
    return A * np.cos(theta*2*np.pi*3 + C) #+ B
initial_guess = [1.0, 1.7]  # Initial guess for A, C, B
...
for j in range(NBS):
    ...
    for lamb in range(len(lambda_id)):
        ...
        for t in range(len(pulse_rot)):
            ...
            # obs_selected_BS = np.random.choice(obs_selected, size=200, replace=True)
...
# plt.errorbar(LAMBDAS,amp_mean, amp_std, fmt='o', color='#26557d', capsize=0, markersize=2, alpha = 1, elinewidth=1)
# plt.errorbar(LAMBDAS,amp_mean, amp_std, fmt='s', color='#ef4870', capsize=0, markersize=2, alpha = 1, elinewidth=1)
plt.errorbar(LAMBDAS,amp_mean, amp_std, fmt='v', color='#ffd168', capsize=0, markersize=2, alpha = 1, elinewidth=1)

plt.xlim([-1.05,1.05])
plt.ylim([0.2,1])
plt.tick_params(axis='both', which='major', labelsize=6)
# plt.savefig('figsm_0p3_obs_good_cosfit1.pdf')
plt.show()

# dist0 = (amp_mean[0]-F[10])/amp_std[0]
# dist2 = (amp_mean[2]-F[30])/amp_std[2]
# print(f'distance/std0 is {dist0}')
