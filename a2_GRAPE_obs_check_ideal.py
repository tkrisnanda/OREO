...
ind = 0
data_dr = np.load(rf"C:\Users\tanju\Dropbox\PY\OREO\xquadD6\{ind}\waves.npz", "r")
...
Gtot = 2*Utotcav.T.conj()@Utotcav - np.eye(cdim)
...
x_op = ((C+Cd)/np.sqrt(2))
# x_op = -1j*((C-Cd)/np.sqrt(2))
# x_op = x_op@x_op

Ntr = 200
x_id = np.zeros(Ntr, dtype = float)
x_sim = np.zeros(Ntr, dtype = float)
for j in range(Ntr):
    ...
    x_id[j] = np.real(np.trace(x_op@rho))
    #the evolution
    rhot = Utot@rho@Utot.T.conj()
    pg = np.real( np.trace(np.kron(ug@ug.T.conj(),np.eye(cdim))@rhot) )
    x_sim[j] = (2*pg-1)*np.real(f)

plt.plot(x_id, x_sim,'ok')
plt.plot(x_id, x_id,'r-')
plt.xlabel('x_id')
plt.ylabel('x_sim')
