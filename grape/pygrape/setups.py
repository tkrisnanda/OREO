...
class Setup(object):
    ...
    def __init__(self, H0, Hcs, c_ops=None, loss_vec=None, sparse=False):
        ...
        self.cdim = H0.dims[0][0]
        self.qdim = H0.dims[0][1]
...
class UnitarySetup_cav(Setup):
    ...
    .. math::
        ...
        \max_\epsilon \big|\text{Tr}[U_\text{target} U(\epsilon)^\dagger]\big|
    """
    def __init__(self, H0, Hcs, U_target, cdim, c_ops=None, gauge_ops=None):
        super(UnitarySetup_cav, self).__init__(H0, Hcs, c_ops=c_ops)
        self.U_target = self.from_qobj(U_target)
        self.cdim_trun = U_target.shape[0]
        self.cdim = cdim
        self.gauge_ops = None
        if gauge_ops is not None:
            self.gauge_ops = self.map_from_qobj(gauge_ops)
    ...
    def get_fids(self, controls, aux_params, dt):
        return prop_fidelity_cav(
            self.cdim, self.cdim_trun, controls, self.H0, self.Hcs, self.U_target, aux_params, self.gauge_ops, dt,
            hermitian=self.hermitian, loss_vec=self.loss_vec
        )

    def set_dtype(self, dtype):
        super(UnitarySetup_cav, self).set_dtype(dtype)
        self.U_target = self.U_target.astype(dtype)
        if self.gauge_ops is not None:
            self.gauge_ops = self.gauge_ops

class ObservableSetup(Setup):
    r"""Optimize a problem of the form

    .. math::

        \max_\epsilon \big|\text{Tr}[G_\text{target} (U(\epsilon)^\dagger P U(\epsilon))^dagger ]\big|
    """
    def __init__(self, H0, Hcs, G_target, Obs, c_ops=None, gauge_ops=None):
        super(ObservableSetup, self).__init__(H0, Hcs, c_ops=c_ops)
        self.U_target = self.from_qobj(G_target)
        self.Obs = self.from_qobj(Obs)
        self.cdim_trun = G_target.shape[0]
        self.gauge_ops = None
        if gauge_ops is not None:
            self.gauge_ops = self.map_from_qobj(gauge_ops)
    ...
    def get_fids(self, controls, aux_params, dt):
        return obs_fidelity(
            self.cdim_trun, controls, self.H0, self.Hcs, self.U_target, self.Obs, aux_params, self.gauge_ops, dt,
            hermitian=self.hermitian, loss_vec=self.loss_vec
        )

    def set_dtype(self, dtype):
        super(ObservableSetup, self).set_dtype(dtype)
        self.U_target = self.U_target.astype(dtype)
        if self.gauge_ops is not None:
            self.gauge_ops = self.gauge_ops

class ObservableSetup_new(Setup):
    r"""Optimize a problem of the form

    .. math::

        \max_\epsilon  1 - np.sum(np.abs( [ G_\text{target} - (U(\epsilon)^\dagger P U(\epsilon)) ] )^2), ^2 is element-wise
    """
    def __init__(self, H0, Hcs, G_target, Obs, c_ops=None, gauge_ops=None):
        super(ObservableSetup_new, self).__init__(H0, Hcs, c_ops=c_ops)
        self.U_target = self.from_qobj(G_target)
        self.Obs = self.from_qobj(Obs)
        self.cdim_trun = G_target.shape[0]
        self.gauge_ops = None
    ...
    def get_fids(self, controls, aux_params, dt):
        return obs_fidelity_new(
            self.cdim_trun, controls, self.H0, self.Hcs, self.U_target, self.Obs, aux_params, self.gauge_ops, dt,
            hermitian=self.hermitian, loss_vec=self.loss_vec
        )

    def set_dtype(self, dtype):
        super(ObservableSetup_new, self).set_dtype(dtype)
        self.U_target = self.U_target.astype(dtype)
        if self.gauge_ops is not None:
            self.gauge_ops = self.gauge_ops

class ObservableSetup_new_gpar(Setup):
    r"""Optimize a problem of the form

    .. math::

        \max_\epsilon  1 - np.sum(np.abs( [ G_\text{target} - (<g|U(\epsilon)^\dagger|g> P <g|U(\epsilon)|g>) ] )^2), ^2 is element-wise
    """
    def __init__(self, H0, Hcs, G_target, Obs, c_ops=None, gauge_ops=None):
        super(ObservableSetup_new_gpar, self).__init__(H0, Hcs, c_ops=c_ops)
        self.U_target = self.from_qobj(G_target)
        self.Obs = self.from_qobj(Obs)
        self.cdim_trun = G_target.shape[0]
        self.cdim = Obs.shape[0]
        self.gauge_ops = None
        if gauge_ops is not None:
            self.gauge_ops = self.map_from_qobj(gauge_ops)
    ...
    def get_fids(self, controls, aux_params, dt):
        return obs_fidelity_new_gpar(
            self.cdim_trun, self.cdim, controls, self.H0, self.Hcs, self.U_target, self.Obs, aux_params, self.gauge_ops, dt,
            hermitian=self.hermitian, loss_vec=self.loss_vec
        )

    def set_dtype(self, dtype):
        super(ObservableSetup_new_gpar, self).set_dtype(dtype)
        self.U_target = self.U_target.astype(dtype)
        if self.gauge_ops is not None:
            self.gauge_ops = self.gauge_ops

class ObservableSetup_new_geonly(Setup):
    r"""Optimize a problem of the form
    ...
    .. math::
        ...
        \max_\epsilon  1 - np.sum(np.abs( [ G_\text{target} - (<g|U(\epsilon)^\dagger|g> P <g|U(\epsilon)|g>) ] )^2), ^2 is element-wise
    """
    def __init__(self, H0, Hcs, G_target, cdim, c_ops=None, gauge_ops=None):
        super(ObservableSetup_new_geonly, self).__init__(H0, Hcs, c_ops=c_ops)
        self.U_target = self.from_qobj(G_target)
        # self.Obs = self.from_qobj(Obs)
        self.cdim_trun = G_target.shape[0]
        self.cdim = cdim#Obs.shape[0]
        self.gauge_ops = None
        if gauge_ops is not None:
            self.gauge_ops = self.map_from_qobj(gauge_ops)
    ...
    def get_fids(self, controls, aux_params, dt):
        return obs_fidelity_new_geonly(
            self.cdim_trun, self.cdim, controls, self.H0, self.Hcs, self.U_target, aux_params, self.gauge_ops, dt,
            hermitian=self.hermitian, loss_vec=self.loss_vec
        )
...
class ObservableSetup_new_geonly_proj(Setup):
    r"""Optimize a problem of the form
    ...
    """
    def __init__(self, H0, Hcs, G_target, cdim, c_ops=None, gauge_ops=None):
        super(ObservableSetup_new_geonly_proj, self).__init__(H0, Hcs, c_ops=c_ops)
        self.U_target = self.from_qobj(G_target)
        # self.Obs = self.from_qobj(Obs)
        self.cdim_trun = G_target.shape[0]
        self.cdim = cdim#Obs.shape[0]
    ...
    def get_fids(self, controls, aux_params, dt):
        return obs_fidelity_new_geonly_proj(
            self.cdim_trun, self.cdim, controls, self.H0, self.Hcs, self.U_target, aux_params, self.gauge_ops, dt,
            hermitian=self.hermitian, loss_vec=self.loss_vec
        )
...
class ObservableSetup_new_geonly_2proj(Setup):
    r"""Optimize a problem of the form
    ...
    .. math::
        ...
        \max_\epsilon  1 - np.sum(np.abs( [ G_\text{target} - (<g|U(\epsilon)^\dagger|g> P <g|U(\epsilon)|g>) ] )^2), ^2 is element-wise
    """
    def __init__(self, H0, Hcs, G_target, G_target2, cdim, c_ops=None, gauge_ops=None):
        super(ObservableSetup_new_geonly_2proj, self).__init__(H0, Hcs, c_ops=c_ops)
        self.U_target = self.from_qobj(G_target)
        self.U_target2 = self.from_qobj(G_target2)
        # self.Obs = self.from_qobj(Obs)
        self.cdim_trun = G_target.shape[0]
        self.cdim = cdim#Obs.shape[0]
        self.gauge_ops = None
    ...
    def __getitem__(self, item):
        return [self.H0, self.Hcs, self.U_target, self.U_target2][item]

    def get_fids(self, controls, aux_params, dt):
        return obs_fidelity_new_geonly_2proj(
            self.cdim_trun, self.cdim, controls, self.H0, self.Hcs, self.U_target, self.U_target2, aux_params, self.gauge_ops, dt,
            hermitian=self.hermitian, loss_vec=self.loss_vec
        )
    ...
    def set_dtype(self, dtype):
        super(ObservableSetup_new_gpar, self).set_dtype(dtype)
        self.U_target = self.U_target.astype(dtype)
        self.U_target2 = self.U_target2.astype(dtype)
        if self.gauge_ops is not None:
            self.gauge_ops = self.gauge_ops
...
def prop_fidelity_from_U(U, dUs, d_g_Us, U_target):
    # cdim = 20
    # Utrun = np.zeros([2*ctrun,2*ctrun], dtype=np.complex_)
    # Utrun[0:ctrun,0:ctrun] = U[0:ctrun,0:ctrun]
    # Utrun[0:ctrun,ctrun:ctrun*2] = U[0:ctrun,cdim:cdim+ctrun]
    # Utrun[ctrun:ctrun*2,0:ctrun] = U[cdim:cdim+ctrun,0:ctrun]
    # Utrun[ctrun:ctrun*2,ctrun:ctrun*2] = U[cdim:cdim+ctrun,cdim:cdim+ctrun]

    # ss = dUs.shape
    # dUstrun = np.zeros([ss[0],ss[1],2*ctrun,2*ctrun])
    # dUstrun[:,:,0:ctrun,0:ctrun] = dUs[:,:,0:ctrun,0:ctrun]
    # dUstrun[:,:,0:ctrun,ctrun:ctrun*2] = dUs[:,:,0:ctrun,cdim:cdim+ctrun]
    # dUstrun[:,:,ctrun:ctrun*2,0:ctrun] = dUs[:,:,cdim:cdim+ctrun,0:ctrun]
    # dUstrun[:,:,ctrun:ctrun*2,ctrun:ctrun*2] = dUs[:,:,cdim:cdim+ctrun,cdim:cdim+ctrun]
    # print(U_target.shape)
    # print(U.shape)
    overlap = 1 - np.sum(np.abs(U_target - U)**2)
    d_overlaps = np.real(np.sum((dUs*(np.conj(U_target)-np.conj(U)) + (U_target-U)*np.conj(dUs)), axis=(2,3)))
...
def prop_fidelity_cav(cdim, cdim_trun, controls, H_drift, H_controls, U_target, gauge_vals, gauge_ops, dt=1,
                  hermitian=True, loss_vec=None):
    """
    Get the total propagator as well as the fidelity to a given target
    defined as abs(Tr(U_target . U.conj().T)) and the gradient of the fidelity
    ...
    return prop_fidelity_from_U_cav(cdim, cdim_trun, tot_prop, d_tot_props, d_g_props, U_target)

# def prop_fidelity_from_U_cav(cdim_trun, U, dUs, d_g_Us, U_target):
#     # D = U_target.shape[0]
#     U = U[0:cdim_trun,0:cdim_trun]
#     dUs = dUs[:,:,0:cdim_trun,0:cdim_trun]
#     norm = np.sum(abs(U_target)**2)
#     overlap = np.sum(U_target.conj() * U) / norm
#     d_overlaps = np.sum(U_target.conj() * dUs, axis=(2,3)) / norm
#     fid = abs(overlap)
...
def prop_fidelity_from_U_cav(cdim, cdim_trun, U, dUs, d_g_Us, U_target):
    U = U[0:cdim_trun,0:cdim_trun]
    dUs = dUs[:,:,0:cdim_trun,0:cdim_trun]
    # U = U[0:cdim_trun,cdim:cdim+cdim_trun]
    # dUs = dUs[:,:,0:cdim_trun,cdim:cdim+cdim_trun]
    
    overlap = 1 - np.sum(np.abs(U_target - U)**2)

    d_overlaps = np.real(np.sum((dUs*(np.conj(U_target)-np.conj(U)) + (U_target-U)*np.conj(dUs)), axis=(2,3)))
    # d_overlaps = (np.sum((dUs*(np.conj(U_target)-np.conj(U)) + (U_target-U)*np.conj(dUs)), axis=(2,3)))
...
# def prop_fidelity_from_U_cav(cdim, cdim_trun, U, dUs, d_g_Us, U_target):
#     U_target_re = U_target.real
#     U_target_im = U_target.imag

#     # U_re = U[0:cdim_trun,0:cdim_trun].real
#     # dUs_re = dUs[:,:,0:cdim_trun,0:cdim_trun].real
#     # U_im = U[0:cdim_trun,0:cdim_trun].imag
#     # dUs_im = dUs[:,:,0:cdim_trun,0:cdim_trun].imag
#     U_re = U[0:cdim_trun,cdim:cdim+cdim_trun].real
#     dUs_re = dUs[:,:,0:cdim_trun,cdim:cdim+cdim_trun].real
#     U_im = U[0:cdim_trun,cdim:cdim+cdim_trun].imag
#     dUs_im = dUs[:,:,0:cdim_trun,cdim:cdim+cdim_trun].imag
    
#     overlap = 1 - np.sum(np.abs(U_target_re - U_re)**2) - np.sum(np.abs(U_target_im - U_im)**2)

#     d_overlaps = np.real(np.sum((dUs_re*(np.conj(U_target_re)-np.conj(U_re)) + (U_target_re-U_re)*np.conj(dUs_re)), axis=(2,3))) + np.real(np.sum((dUs_im*(np.conj(U_target_im)-np.conj(U_im)) + (U_target_im-U_im)*np.conj(dUs_im)), axis=(2,3)))
#     # d_overlaps = (np.sum((dUs*(np.conj(U_target)-np.conj(U)) + (U_target-U)*np.conj(dUs)), axis=(2,3)))
...
def obs_fidelity(cdim_trun, controls, H_drift, H_controls, G_target, Obs, gauge_vals, gauge_ops, dt=1,
                  hermitian=True, loss_vec=None):
    """
    Get the total observable as well as the fidelity to a given target
    defined as abs(Tr(G_target . (U.conj().T @ P @ U)^dagger  )) and the gradient of the fidelity
    with respect to the controls
    """
    H_drift = dt * H_drift
    H_controls = dt * np.array(H_controls)
    ...
    return prop_fidelity_from_UdPU(cdim_trun, tot_prop, d_tot_props, d_g_props, G_target, Obs)

def prop_fidelity_from_UdPU(cdim_trun, U, dUs, d_g_Us, G_target, Obs):
    #Obs lives in qubit-cavity space
    G = np.conj(np.transpose(U))@Obs@U
    G = G[0:cdim_trun,0:cdim_trun]

    norm = np.sum(abs(G_target)**2)
    overlap = np.sum(G_target.conj() * G) / norm

    dUsd = np.conj(np.transpose(dUs, axes=(0,1,3,2)))
    # dUs = dUs[:,:,0:cdim_trun,0:cdim_trun]

    #the derivatives of G
    dGs = dUsd @ Obs@U + np.conj(np.transpose(U))@Obs @ dUs
    dGs = dGs[:,:,0:cdim_trun,0:cdim_trun]

    d_overlaps = np.sum(G_target.conj() * dGs, axis=(2,3)) / norm
    ...
    if len(d_g_Us) == 0:
        ...
        d_g_overlaps = np.sum(G_target.conj() * d_g_Us, axis=(1,2)) / norm
        d_g_fid = (overlap.real*d_g_overlaps.real + overlap.imag*d_g_overlaps.imag) / fid
    return G, fid, d_fid, d_g_fid

def obs_fidelity_new(cdim_trun, controls, H_drift, H_controls, G_target, Obs, gauge_vals, gauge_ops, dt=1,
                  hermitian=True, loss_vec=None):
    """
    Get the total observable as well as the fidelity to a given target
    defined as 1 - sum( abs(G_target - (U.conj().T @ P @ U)  )^2 ) and the gradient of the fidelity
    with respect to the controls
    """
    H_drift = dt * H_drift
    H_controls = dt * np.array(H_controls)
    ...
    return prop_fidelity_from_UdPU_new(cdim_trun, tot_prop, d_tot_props, d_g_props, G_target, Obs)

def prop_fidelity_from_UdPU_new(cdim_trun, U, dUs, d_g_Us, G_target, Obs):
    #Obs lives in qubit-cavity space
    G = np.conj(np.transpose(U))@Obs@U
    G = G[0:cdim_trun,0:cdim_trun]

    overlap = 1 - np.sum(np.abs(G_target - G)**2)
    ...
    # d_overlaps = - np.sum(np.abs(G_target - dGs), axis=(2,3))
    d_overlaps = np.real(np.sum((dGs*(np.conj(G_target)-np.conj(G)) + (G_target-G)*np.conj(dGs)), axis=(2,3)))
    # d_overlaps = np.sum(( (G_target.conj())*dGs), axis=(2,3))
    ...
    if len(d_g_Us) == 0:
        ...
        d_g_overlaps = 1 - np.linalg.norm(G_target - d_g_Us, axis=(1,2))/2
        d_g_fid = d_g_overlaps
    return G, fid, d_fid, d_g_fid

def obs_fidelity_new_gpar(cdim_trun, cdim, controls, H_drift, H_controls, G_target, Obs, gauge_vals, gauge_ops, dt=1,
                  hermitian=True, loss_vec=None):
    """
    Get the total observable as well as the fidelity to a given target
    defined as 1 - sum( abs(G_target - (U.conj().T @ P @ U)  )^2 ) and the gradient of the fidelity
    ...
    return prop_fidelity_from_UdPU_new_gpar(cdim_trun, cdim, tot_prop, d_tot_props, d_g_props, G_target, Obs)

def prop_fidelity_from_UdPU_new_gpar(cdim_trun, cdim, U, dUs, d_g_Us, G_target, Obs):
    #Obs lives in cavity space
    Ucav = U[0:cdim,0:cdim]# this is <g|U|g>, lives in cavity space
    G = np.conj(np.transpose(Ucav))@Obs@Ucav
    G = G[0:cdim_trun,0:cdim_trun]

    overlap = 1 - np.sum(np.abs(G_target - G)**2)

    # dUsd = np.conj(np.transpose(dUs, axes=(0,1,3,2)))
    dUscav = dUs[:,:,0:cdim,0:cdim]
    dUscavd = np.conj(np.transpose(dUscav, axes=(0,1,3,2)))
    # dUs = dUs[:,:,0:cdim_trun,0:cdim_trun]

    #the derivatives of G
    dGs = dUscavd @ Obs@Ucav + np.conj(np.transpose(Ucav))@Obs @ dUscav
    dGs = dGs[:,:,0:cdim_trun,0:cdim_trun]
...
def obs_fidelity_new_geonly(cdim_trun, cdim, controls, H_drift, H_controls, G_target, gauge_vals, gauge_ops, dt=1,
                  hermitian=True, loss_vec=None):
    """
    Get the total observable as well as the fidelity to a given target
    defined as 1 - sum( abs(G_target - (U.conj().T @ P @ U)  )^2 ) and the gradient of the fidelity
    ...
    return prop_fidelity_from_UdPU_new_geonly(cdim_trun, cdim, tot_prop, d_tot_props, d_g_props, G_target)

def prop_fidelity_from_UdPU_new_geonly(cdim_trun, cdim, U, dUs, d_g_Us, G_target):
    #Obs lives in cavity space
    Ucav = U[0:cdim,0:cdim]# this is <g|U|g>, lives in cavity space
    G = 2*np.conj(np.transpose(Ucav))@Ucav - np.eye(cdim)
    G = G[0:cdim_trun,0:cdim_trun]
    ...
    #the derivatives of G
    dGs = 2*( dUscavd @Ucav + np.conj(np.transpose(Ucav)) @ dUscav )
    dGs = dGs[:,:,0:cdim_trun,0:cdim_trun]
...
def obs_fidelity_new_geonly_proj(cdim_trun, cdim, controls, H_drift, H_controls, G_target, gauge_vals, gauge_ops, dt=1,
                  hermitian=True, loss_vec=None):
    """
    Get the total observable as well as the fidelity to a given target
    defined as 1 - sum( abs(G_target - (U.conj().T @ P @ U)  )^2 ) and the gradient of the fidelity
    ...
    return prop_fidelity_from_UdPU_new_geonly_proj(cdim_trun, cdim, tot_prop, d_tot_props, d_g_props, G_target)

def prop_fidelity_from_UdPU_new_geonly_proj(cdim_trun, cdim, U, dUs, d_g_Us, G_target):
    #Obs lives in cavity space
    Ucav = U[0:cdim,0:cdim]# this is <g|U|g>, lives in cavity space
    # Ucav = U[cdim:2*cdim,0:cdim]# this is <e|U|g>, lives in cavity space
    G = Ucav.T.conj()@Ucav
    G = G[0:cdim_trun,0:cdim_trun]

    overlap = 1 - np.sum(np.abs(G_target - G)**2)

    dUscav = dUs[:,:,0:cdim,0:cdim]#for gg
    # dUscav = dUs[:,:,cdim:2*cdim,0:cdim]#for eg
    dUscavd = np.conj(np.transpose(dUscav, axes=(0,1,3,2)))

    #the derivatives of G
    dGs = ( dUscavd @Ucav + Ucav.T.conj() @ dUscav )
    dGs = dGs[:,:,0:cdim_trun,0:cdim_trun]
...
def obs_fidelity_new_geonly_2proj(cdim_trun, cdim, controls, H_drift, H_controls, G_target, G_target2, gauge_vals, gauge_ops, dt=1,
                  hermitian=True, loss_vec=None):
    """
    Get the total observable as well as the fidelity to a given target
    defined as 1 - sum( abs(G_target - (U.conj().T @ P @ U)  )^2 ) and the gradient of the fidelity
    ...
    return prop_fidelity_from_UdPU_new_geonly_2proj(cdim_trun, cdim, tot_prop, d_tot_props, d_g_props, G_target, G_target2)

def prop_fidelity_from_UdPU_new_geonly_2proj(cdim_trun, cdim, U, dUs, d_g_Us, G_target, G_target2):
    #Obs lives in cavity space

    Ucav = U[0:cdim,0:cdim]# this is <g|U|g>, lives in cavity space
    Ucav2 = U[cdim:2*cdim,0:cdim]# this is <e|U|g>, lives in cavity space

    G = Ucav.T.conj()@Ucav
    G = G[0:cdim_trun,0:cdim_trun]

    G2 = Ucav2.T.conj()@Ucav2
    G2 = G2[0:cdim_trun,0:cdim_trun]

    overlap = 1 - np.sum(np.abs(G_target - G)**2) - np.sum(np.abs(G_target2 - G2)**2)

    dUscav = dUs[:,:,0:cdim,0:cdim]#for gg
    dUscavd = np.conj(np.transpose(dUscav, axes=(0,1,3,2)))

    dUscav2 = dUs[:,:,cdim:2*cdim,0:cdim]#for eg
    dUscavd2 = np.conj(np.transpose(dUscav2, axes=(0,1,3,2)))

    #the derivatives of G
    dGs = ( dUscavd @Ucav + Ucav.T.conj() @ dUscav )
    dGs = dGs[:,:,0:cdim_trun,0:cdim_trun]

    dG2s = ( dUscavd2 @Ucav2 + Ucav2.T.conj() @ dUscav2 )
    dG2s = dG2s[:,:,0:cdim_trun,0:cdim_trun]

    # d_overlaps = - np.sum(np.abs(G_target - dGs), axis=(2,3))
    d_overlaps = np.real(np.sum((dGs*(np.conj(G_target)-np.conj(G)) + (G_target-G)*np.conj(dGs)), axis=(2,3))) + np.real(np.sum((dG2s*(np.conj(G_target2)-np.conj(G2)) + (G_target2-G2)*np.conj(dG2s)), axis=(2,3)))
    # d_overlaps = np.sum(( (G_target.conj())*dGs), axis=(2,3))
...
def taylor_states_fidelity(controls, H0, Hcs, inits, finals, dt=1,
                           gauge_vals=None, gauge_ops=None,
...
if __name__ == '__main__':
    ...
    _, c1, g1, g2 = prop_fidelity_cav(controls, H0, Hcs, U_target, gauge_vals, gauge_ops)
    # print g1
    # print g2
    gauge_vals[idx] += 1e-7
    # controls[0, 5] += 1e-7
    _, c2, _, _ = prop_fidelity_cav(controls, H0, Hcs, U_target, gauge_vals, gauge_ops)
    g3 = 1e7 * (c2 - c1)
    # print g1[0, 5]
    print(g2[idx])
    print(g3)
