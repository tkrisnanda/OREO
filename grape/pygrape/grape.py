...
def run_grape(init_controls, setups, penalty_fns=None, reporter_fns=None,
    ...
    """
    Run BFGS in order to minimize the infidelity of unitary implemented by the controls
    ...
    setups : List of tuples
        Each tuple consists of either 3 elements (H drift, H controls, U target)
        or 4 elements (H drift, H controls, init states, final states)
        ...
        If any of these elements are qutip Qobjects automatic conversion will be attempted
    penalty_fns : List of callables
        Each function should take the controls as an argument and should
        return a (cost, gradient) tuple
    reporter_fns : List of callables
        Functions used for status reporting. The argument names are extracted
        from the function itself, and used to supply the necessary data to the
        function. The supported names are as follows

        - sim_controls: Controls which are being simulated
        - dt: Time step
        - n_ss: Number of subsamples
        - raw_controls: Controls which are being manipulated by the minimizer
        - awg_controls: Controls which would be sent to AWG
        - shape_func: Shape profile function
        - response: Impulse response derived from filter function
        - setups: List of setups
        - props: Propagators / final_states for each setup if calculated
        - fids: Fidelity for each setup
        - fid_grads: derivative of the fidelity for each setup
        - pen_cost: penalty costs
        - pen_grads: deriviative for penalty costs
        - tot_cost: total cost
        - tot_grad: total derivative
        - outdir: current output directory
    outdir : str
        Output directory. Created if it does not exist. Output is put in a numbered
        subdirectory of outdir, which prevents overwriting old results.
    dt : float
        time step size of the control waveforms
    n_ss : int
        factor by which to sub-sample the controls
    filt_func : callable
        a filter function that returns the response versus frequency,
        e.g. a Gaussian lambda fs: exp(-fs**2/(2*sigma**2))
    test_cost : int or bool
        if nonzero, run specified number of tests of the gradient to ensure its accuracy.
        The minimization is not performed
    shape_sigma : float
        Specifies the width of the shape function, which forces the amplitude to go to zero smoothly
        at the beginning and end of the pulse.
    n_proc : int
        If using multiple setups, multiple processors can be used to run each setup in parallel.
        This specifies the number of processes to use. There is no advantage to setting this number
        larger than the number of setups.
    discrepancy_penalty : float
        If using multiple setups, penalize the discrepancy sum(F_k - mean({F_k}) of the fidelitie F_k
    dtype : numpy dtype
        Set the dtype used to do calculations. Set to np.complex64 if speed is valued over precision
    opts : dict
        additional arguments are passed to minimizer.
        Run 'scipy.optimize.show_options('minimize', 'l-bfgs-b') to see options.
    """
    ...
    def get_report_data(controls, force_report=False):
        ...
        if raw_controls.size == n_ctrls * plen:
            raw_controls = raw_controls.reshape((n_ctrls, plen))
        else:
            warn(
                f'raw_controls size ({raw_controls.size}) cannot be reshaped into n_ctrls, plen {n_ctrls, plen}. '\
                "Kai Xiang's guess attempt to fix this: the freq_range specification changed the number of controls "\
                "by truncating it, so the tighter the bounds, the lesser points you have. I will hotfix this by adding an extra line of code"\
                )
...
#            d_discrepancy_cost = 2 * discrepancy_penalty * sum(df * d_fid for df, d_fid in zip(discrepancies, fid_grads[discrepancy_idcs]))
    ...
    if not eval_once:
        ...
        if freq_range is not None:
            print("WARNING: Kai Xiang inserting a hotfix here for the error")
            x = get_controls(x)
    ...
    for k, v in list(data.items()):
        ...
        if isinstance(v, dict):
            ...
            try:
                ...
                Warning("The library is trying to save the optimisation simulation and the validation simulation"\
                                ", which have different Hilbert spaces and thus is throwing an error. This is currently quickpathed by "\
                                "having a try-except clause at Line 481 in grape.py. ")
            # print k, arr.dtype
