...
class Reporter(object):
    ...
    def __call__(self, force=False, **kwargs):
        if force or self.n_call % self.spacing == 0:
            args = [kwargs[k] for k in inspect.getfullargspec(self.run).args[1:]]
            self.run(*args)
        self.n_call += 1
...
class plot_waves(Reporter):
    ...
    def __init__(self, wave_names, spacing=5, iq_pairs=False, last_only=False):
        ...
        if iq_pairs:
            ...
            self.fig = plt.figure(figsize = (8,3*len(wave_names)))
            gs1 = GridSpec(n_ax, 2)
    ...
    #                ax.clear()
        ...
        if self.iq_pairs:
            ...
            for idx, (ax, wave) in enumerate(zip(wave_axes, full_controls)):
                ...
                ax.plot(wave, 'r-', label="Amplitude")
                ax.set_ylim(min(wave)*11/10, max(wave) * 11/10)
                #ax.legend(prop={'size': 4})
                ax.set_ylabel("Amplitude",fontsize=8)
                ax.set_xlabel(f"Time (units of {dt}ns)",fontsize=8)
            ...
            for idx, ax in enumerate(fft_axes):
                ...
                ax.set_ylim(-max(fft_wave)/10, max(fft_wave) * 11/10)
                #ax.set_yticklabels([])
                #if idx == 0:
                #    ax.set_xticklabels([])

                ax.set_title("Fourier Transform",fontsize = 12)
                ax.set_ylabel("Amplitude",fontsize=8)
                ax.set_xlabel("Frequency (MHz)",fontsize=8)
                
        for ax, wave_name in zip(wave_axes, self.wave_names):
            ax.set_title(wave_name,fontsize = 12)#x=-0.075, y=0.25)
...
class plot_trajectories(Reporter):
    ...
    def run(self, outdir, sim_controls, aux_params, dt, n_ss):
        ...
        if len(self.setup.inits) == 1: axes = [axes,]
