...
filepath = r"C:\Users\tanju\Dropbox\PY\Data\OREO\fig2 data\x2\11-39-58_ObservableMeasPS.hdf5"
...
data_ave = (1-scale_q(data_ave))*f#since x2 is positive we process pg only
...
def Bootstrap(ind):
    ...
    for j in range(NBS):
        for k in range(le):
            data_BS = np.random.choice(data_1d[:,k], size=1000, replace=True)#data_size[0]
            data_BS_all[j,k] = np.mean(data_BS)#pe

    data_BS_all = (1-scale_q(data_BS_all))*f
...
data_id = np.load(r'C:\Users\tanju\Dropbox\PY\OREO\x2_ideal.npz')['obs']
...
plt.tick_params(axis='both', which='major', labelsize=6)
plt.xlim([-1.05,1.05])
plt.yticks([0.5, 1.5, 2.5], ['1','1','1'])
# plt.savefig('fig2b_bot.pdf')
plt.show()
